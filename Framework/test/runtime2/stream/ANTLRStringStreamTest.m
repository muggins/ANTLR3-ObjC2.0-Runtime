...
-(void) testInitWithInput
{
	...
	ANTLRStringStream *stream = [[ANTLRStringStream alloc] initWithInput:input];
	NSString *subString = [stream substring:0 stop:10];
...
{
	ANTLRStringStream *stream = [[ANTLRStringStream new] initWithInput:@"This is a string used for input"];
	[stream consume];
...
{
	ANTLRStringStream *stream = [[ANTLRStringStream new] initWithInput:@"This is a string\nused for input"];
...
-(void) testLAEOF
{
	ANTLRStringStream *stream = [[ANTLRStringStream new] initWithInput:@"This is a string\nused for input"];
	BOOL eofFound = NO;
	for (NSInteger i = 0; i < [stream size]; i++)
	{
	...
	STAssertTrue(eofFound, @"EOF Was not found in stream");
	[stream release];
}
